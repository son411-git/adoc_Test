.Fog Signal의 정의
[cols="3,2,1,1", options="header"]
|===
4+h|*Definition:* A warning signal transmitted by a vessel, or aid to navigation, during periods of low visibility. Also, the device producing such a signal.
4+h|*Primitives:* point
h|Attribute h|Acronym h|Type h|Mult.

|span:E[category of fog signal]|CATFOG|EN|1,1
|**feature name**||C|0,*
|    span:E[language]||(S)TE|1,1
|    span:E[name]|OBJNAM/NOBJNM|(S)TE|1,1
|    name usage||(S)EN|0,1
|**fixed date range**||C|0,1
|    date end|DATEND|(S)TD|0,1
|    date start|DATSTA|(S)TD|0,1
|interoperability identifier||URN|0,1
|**periodic date range**||C|0,*
|    span:E[date end]|DATEND|(S)TD|1,1
|    span:E[date start]|DATSTA|(S)TD|1,1
|signal frequency|SIGFRQ|IN|0,1
|signal generation|SIGGEN|EN|0,1
|signal group|SIGGRP|TE|0,1
|signal period|SIGPER|RE|0,1
|**signal sequence**|SIGSEQ|C|0,*
|    span:E[signal duration]||(S)RE|1,1
|    span:E[signal status]||(S)EN|1,1
|status|STATUS|EN|0,*
|value of maximum range|VALMXR|RE|0,1
|scale minimum|SCAMIN|IN|0,1
|**information**|INFORM|C|0,*
|    file locator||(S)TE|0,1
|    file reference|TXTDSC/NTXTDS|(S)TE|0,1
|    headline||(S)TE|0,1
|    span:E[language]||(S)TE|1,1
|    text|INFORM/NINFOM|(S)TE|0,1
|===

include::../features_rules/FogSignal_rules.adoc[tag=FogSignal]
