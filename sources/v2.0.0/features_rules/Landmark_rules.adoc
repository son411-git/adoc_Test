...
//
* 폐허가 된 span:F[Landmark]는 양호한 상태의 객체와 동일하게 입력하고, 속성 span:A[condition] = 2 span:D[ruined]를 입력해야 함
//
...
//
* span:A[special purpose mark] 는 span:F[Landmark]가 지도선(leading line), 피험선(clearing line)의 전방 또는 후방 표지로 사용되는 경우에만 사용해야 하며, 각각 span:A[special purpose mark] = 16 span:D[leading mark] / 41 span:D[clearing mark]로 입력해야 함
...
| span:F[Landmark] +
span:A[special purpose mark] = 16 span:D[leading mark]
| span:F[Landmark] +
span:A[special purpose mark] = 41 span:D[clearing mark]
| span:F[Navigation Line] +
span:A[category of navigation line] = 3 span:D[leading line bearing a recommended track]
| span:F[Navigation Line] +
span:A[category of navigation line] = 1 span:D[clearing line]
...
//
* span:A[category of landmark]의 값 중에서 26 span:D[bridge] 및 27 span:D[dam]은 span:F[Landmark][P]인 경우에만 사용해야 하며, 항해 가능한 수역 위에는 입력하지 않아야 함.
...
|span:E[category of landmark]|CATLMK|EN|1,*| 17 span:D[tower]
...
|function|FUNCTN|EN|0,*| 33 span:D[light support]
|nature of construction|NATCON|EN|0,*| 1 span:D[masonry]
|vertical length|VERLEN|RE|0,1| 26
|span:E[visual prominence]|CONVIS|EN|1,1| 1 span:D[visually conspicuous]
|===
