...
- insert(기재), delete(삭제), modify(수정)인 경우 피처 형태에 따라 변경된 부분에만 span:F[Update Information]을 입력하고 span:R[Update Information] 관계 설정(<<UPDATE_INFO,※참고>>)
- move(이동)인 경우 example 4를 참조하여 3개의 span:F[Update Information] 피처를 만들고 관계 설정
...
////
- span:F[Update Information] 사용 시 더 많은 추가정보의 입력이 필요하다면 span:A[Information]을 사용하여 입력
- 만약 항행통보 등의 이유로 기존에 존재하는 Feature가 삭제되었다면 span:F[Update Information]을 사용하여 삭제 정보를 표기해야 함
- 데이터 셋의 개정 시 span:F[Update Information]을 삭제
- 이전 버전과 동일한 피처가 업데이트되는 경우, 개정이 아니어도 기존의 span:F[Update Information]을 삭제
- 기존 Feature가 삭제되었다는 정보를 표현하기 위해 span:F[Update Information]를 사용한 경우, 관련된 Feature가 이미 삭제되었으므로 span:R[Updated Information] 관계 설정 하지 않음 


===== Example
.UpdateInformation의 인코딩 예시
