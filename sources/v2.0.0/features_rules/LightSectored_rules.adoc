...
* 강도가 다른 분호가 있는 등화의 경우 span:A[light sector] 하위 속성 span:A[light visibility] = (4 : intensified)를 채워야 함
...
* span:A[light charateristic] = (11 : spotlight)을 입력하는 경우, 그 목적은 span:A[information]로 입력해야 함
...
* span:A[vertical datum] = (13 : low water)은 내륙 수로에만 적용되며, 조석이 적용되는 수역에서 등화 높이의 기준 데이터를 나타내기 위해 사용해서는 안 됨
...
** 부분적으로 가려진 분호(암호) (b)는 span:A[light sector]의 하위 속성 span:A[light visibility] = (8 : partially obscured)와 span:A[value of nominal range]를 등화에서 장애물까지의 거리로 설정하여 입력해야 함. 
* 등화와 장애물 사이에 항해 가능한 수역이 없는 경우(위 그림 19-2 (B)의 (e) 참조), 가려진 부분은 **암호**가 아니므로 별도의 span:A[light sector]로 입력하지 않아야 함. 
** 단, 명호의 항해 가능 부분에서 희미한 등화가 보이는 경우에는 span:A[light sector]를 사용하고, 하위 속성 span:A[light visibility] = (3 : faint)로 입력해야 함. 
...
|    span:E[light characteristic]|LITCHR|(S)EN|1,1| 2 : flashing
|**    span:E[light sector]**||(S)C|1,*|
|        span:E[colour]|COLOUR|(S)EN|1,*| 1 : white
...
|**span:E[sector characteristics]**||C|1,*|
|    span:E[light characteristic]|LITCHR|(S)EN|1,1| 7 : isophased
|**    span:E[light sector]**||(S)C|1,*|
|        span:E[colour]|COLOUR|(S)EN|1,*| 1 : white
|**        sector limit**||(S)C|0,1|
|**            span:E[sector limit one]**|SECTR1|(S)C|1,1|
...
|**    span:E[light sector]**||(S)C|1,*|
|        span:E[colour]|COLOUR|(S)EN|1,*| 3 : red
|**        sector limit**||(S)C|0,1|
|**            span:E[sector limit one]**|SECTR1|(S)C|1,1|
...
|**    span:E[light sector]**||(S)C|1,*|
|        span:E[colour]|COLOUR|(S)EN|1,*| 4 : green
|**        sector limit**||(S)C|0,1|
|**            span:E[sector limit one]**|SECTR1|(S)C|1,1|
|                span:E[sector bearing]|SECTR1/SECTR2|(S)RE|1,1| 342
|**            span:E[sector limit two]**|SECTR2|(S)C|1,1|
