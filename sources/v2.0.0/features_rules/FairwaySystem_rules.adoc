...
* 굽이진 경로로 구성된 항로는 대표 객체인 span:F[Fairway System]를 별도로 입력
** span:F[Fairway]의 공간적 범위는 구성 객체들의 전체 범위를 포함해야함
* 객체의 명칭이 없으면 span:F[Fairway System][N]으로 입력
* 객체의 명칭을 알고 있고 ECDIS에서 명칭이 표시되어야 하는 경우, + 
  span:F[Fairway System][S]로 입력하고 명칭은 span:A[feature name]으로 입력
...
|    name usage||(S)EN|0,1| 1 : defalut name display
|**feature name**||C|0,*|
